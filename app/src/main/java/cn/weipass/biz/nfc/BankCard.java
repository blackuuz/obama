...
public class BankCard {
	...
	private static String getName(String id){
		...
		String pre=id.substring(0,6);
		if(NameMap.containsKey(pre)){
			r=NameMap.get(pre);
		}
		return r;
	}
	...
	public static class BankCardInfo {
		...
		public String toString() {
			return "BankCardInfo [type=" + type + ", id=" + id + ", desc=" + desc + ", name=" + name + ", idcard=" + idcard + "]";
		}
	 ...
	 */
	@SuppressLint("NewApi")
	private final static byte[] getBytes(byte[] bs, int tb) {
		int i = 0;
		int j = 0;
		do {
			if (i >= bs.length) {
			...
			j++;
			if (b == tb) {
				return Arrays.copyOfRange(bs, j + 1, bs[j] + (j + 1));
			}
			i++;
		} while (true);
	}

//	private final static byte[] getBytes(byte[] bs, byte sub[]) {
//		for (int i = 0; i < bs.length - sub.length; i++) {
//			int j = 0;
//			while (j < sub.length && sub[j] == bs[i + j]) {
//				j++;
//			}
//			if (j == sub.length) {
//				return Arrays.copyOfRange(bs, j + i + 1, bs[i + sub.length] + (sub.length + i + 1));
//			}
//		}
//		return null;
//	}
	 ...
	 */
	public static BankCardInfo readBankCardInfo(IsoDep c) throws CpuCardException {
		BankCardInfo ri = new BankCardInfo();
		try {
			CmdResult r = null;
			CpuCard.selectFile(c, "1PAY.SYS.DDF01");
			r = CpuCard.doCmd(c, "00B2010C00");
			byte[] tbs = r.rdata;

			String fs = HEX.bytesToHex(getBytes(tbs, 79));
			ri.type = "";
			if ("A000000333010101".equals(fs)) {
				ri.type = "DEBIT_CARD";
			} else if ("A000000333010102".equals(fs)) {
				ri.type = "CREDIT_CARD";
			} else if ("A000000333010103".equals(fs)) {
				ri.type = "QUASI_CREDIT_CARD";
			}
			try {
				ri.desc = new String(getBytes(tbs, 80), "UTF8");
			} catch (Exception e) {
			}
			CpuCard.selectFile(c, HEX.hexToBytes(fs));

//			r = CpuCard.doCmd(c, "00B2010C00");// 00B2010C3C
//			try {
//				ri.name = new String(getBytes(r.rdata, new byte[] { 95, 32 }), "GBK");
//			} catch (Exception e) {
//			}

			r = CpuCard.doCmd(c, "00B2011400");// 00B2011444
			{
				byte[] ts = getBytes(r.rdata, 90);
				ri.id = HEX.bytesToHex(ts);
				ri.id=ri.id.replace("F", "");
			}
			ri.name=getName(ri.id);
//			System.out.println("0101===========");
//			r = CpuCard.doCmd(c, "00B2010C00");
//			r = CpuCard.doCmd(c, "00B2020C00");
//			r = CpuCard.doCmd(c, "00B2030C00");
	//
//			System.out.println("0201===========");
//			r = CpuCard.doCmd(c, "00B2011400");
//			r = CpuCard.doCmd(c, "00B2021400");
//			r = CpuCard.doCmd(c, "00B2031400");
//			r = CpuCard.doCmd(c, "00B2041400");
//			r = CpuCard.doCmd(c, "00B2051400");
