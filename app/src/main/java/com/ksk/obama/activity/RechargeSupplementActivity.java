...
import static com.ksk.obama.R.id.title_name;


public class RechargeSupplementActivity extends BaseSupplementActivity implements IPrintErrorCallback, IPrintSuccessCallback {

    private List<RechargeAgain> list = new ArrayList<>();
    ...
    private void initView() {
        TextView name = (TextView) findViewById(title_name);
        name.setText("充值补单");
    ...
    private void queryDb() {
        ...
        list = DataSupport.findAll(RechargeAgain.class);
    ...
    public void showData(String text, final int n) {
        ...
        try {
            ...
            if (msg.equals("SUCCESS")) {
                ...
                tv_hint.setText("当前有 " + orderYes + "/" + list.size() + " 个订单正在上传,不要退出页面");
                DataSupport.delete(RechargeAgain.class, list.get(n).getId());
    ...
    private synchronized void printInformation(final int n) {
        ...
        RechargeAgain upLoading = list.get(n);
        ...
        printP.add("充值金额:" + upLoading.getPayShould());
        printP.add("实收金额 :" + upLoading.getPayActual());
        printP.add("充后余额 :" + haveMoney);
        printP.add("可用积分 :" + haveIntegral);
        printPage("会员充值补单小票", printP, null, false);
    }
