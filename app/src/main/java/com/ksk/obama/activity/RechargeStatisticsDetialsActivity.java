...
import com.ksk.obama.model.RechargeStatisticsDetials;
...
public class RechargeStatisticsDetialsActivity extends BasePrintActivity implements IPrintErrorCallback{
    ...
    private TextView tv_paya;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_recharge_statistics_detials);
    ...
    private void initView() {
        ...
        tv_paya = (TextView) findViewById(R.id.tv_open_sh);
    }

    private void getDetailInfo() {
        String id = getIntent().getStringExtra("MemberId");
        Map<String, String> map = new HashMap<>();
        map.put("BillType", "充值收费");
        map.put("dbName", SharedUtil.getSharedData(RechargeStatisticsDetialsActivity.this, "dbname"));
        map.put("id", id);
    ...
    private void showHttpData(String json) {
        try {
            ...
            if (tag.equals("SUCCESS")) {
                Gson gson = new Gson();
                RechargeStatisticsDetials recordDel = gson.fromJson(json, RechargeStatisticsDetials.class);
                tv_cardMoney.setText("￥" + recordDel.getN_PayShould());
                tv_name.setText(recordDel.getC_Name() + "");
                tv_time.setText(recordDel.getT_Time() + "");
                tv_mode.setText(recordDel.getPay_way() + "");
                tv_startMoney.setText("￥" + recordDel.getMoney());
                tv_cardJf.setText(recordDel.getN_IntegralAvailable());
                ...
                tv_paya.setText(recordDel.getN_PayActual());
            } else {
                String msg = object.getString("result_errmsg");
                Utils.showToast(RechargeStatisticsDetialsActivity.this, msg);
            }
    ...
    private void printInfo() {
        ...
        list.add("充值金额:" + tv_cardMoney.getText().toString());
        list.add("卡类型:" + tv_cardType.getText().toString());
        list.add("卡号:" + tv_cardNum.getText().toString());
        list.add("姓名:" + tv_name.getText().toString());
        list.add("充值时间:" + tv_time.getText().toString());
        list.add("支付方式:" + tv_mode.getText().toString());
        list.add("实收金额:" + tv_paya.getText().toString());
        list.add("收款单号:" + tv_dj.getText().toString());
        list.add("原金额:" + tv_startMoney.getText().toString());
        list.add("充后金额:" + tv_endMoney.getText().toString());
        list.add("当前积分:" + tv_cardJf.getText().toString());
        list.add("充值店面:" + tv_shopName.getText().toString());
        list.add("操作员:" + tv_user.getText().toString());
        printPage("充值详情",list,null,false);
    }
