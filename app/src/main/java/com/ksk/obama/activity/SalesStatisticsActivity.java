...
import com.ksk.obama.adapter.SalesStatisticsAdapter;
...
public class SalesStatisticsActivity extends BaseActivity implements View.OnClickListener {
    ...
    private List<SalesStatistics.ResultDataBean> list = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lv_data.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position,
                ...
                intent.putExtra("name", list.get(position).getC_GoodsName());
                intent.putExtra("num", list.get(position).getAllNum());
                intent.putExtra("money", list.get(position).getAllmoney());
                intent.putExtra("times", time_s.getText().toString());
                intent.putExtra("timee", time_e.getText().toString());
                intent.setClass(SalesStatisticsActivity.this, SalesStatisticsDetialsActivity.class);
    ...
    private void initView() {
        ...
        tv_title10.setText("商品名");
        tv_title11.setText("数量");
        tv_title12.setText("售价");
        tv_title13.setText("详情");
        adapter = new SalesStatisticsAdapter(SalesStatisticsActivity.this, list);
    ...
    private void getHttpData() {
        ...
        map.put("UserinfoId", SharedUtil.getSharedData(SalesStatisticsActivity.this, "userInfoId"));
        map.put("dbName", SharedUtil.getSharedData(SalesStatisticsActivity.this, "dbname"));
        map.put("STTime", sTTime + " 00:00:00");
        map.put("ENDTime", eNDTime + " 23:59:59");
        postToHttp(NetworkUrl.SALES, map, new IHttpCallBack() {
            @Override
    ...
    private void showHttpData(String jsonText) {
        SalesStatistics statistics = new Gson().fromJson(jsonText, SalesStatistics.class);
        if (statistics.getResult_stadus().equals("SUCCESS")) {
            tv_title0.setText(statistics.getResult_data2().getTypenum() + "条记录");
            tv_title1.setText("数量:" + statistics.getResult_data2().getTypecount());
            tv_title2.setText("收款" + statistics.getResult_data2().getTypemoney() + "元");
            if (statistics.getResult_data() != null) {
                list.addAll(statistics.getResult_data());
                adapter.notifyDataSetChanged();
            } else {
                Utils.showToast(SalesStatisticsActivity.this, "此条件下没有数据");
            }
        } else {
            Utils.showToast(SalesStatisticsActivity.this, statistics.getResult_errmsg());
        }
    }
